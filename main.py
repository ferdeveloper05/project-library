from fastapi import FastAPI, Query, Path, HTTPException
from typing import Annotated
from models import CreateBook, UpdateBook
from uuid import UUID, uuid4
from local_functions import read_book, save_book, delete_book_new, edit_book

app = FastAPI(
    version='1.0.0',
    title='Personal Library Project'
)

books = []

@app.get("/", tags=['Home'])
async def say_hello():
    return {'Hello':'World'}

# Create new book
@app.post('/create-book', tags=['Library'])
async def create_book(book: CreateBook):
    book.id = str(uuid4())
    await save_book(book.model_dump())
    return read_book()

# List Books
@app.get('/list_books', tags=['Library'])
async def list_books() -> list:
    return read_book()

# Search book
@app.get('/view-book/{book_id}/', tags=['Library'])
async def view_book(book_id: UUID) -> dict:
    list_books = read_book()
    for book in list_books: 
        if book['id'] == str(book_id):
            return book

# Update book
@app.put('/update-book/{book_id}/', tags=['Library'])
async def update_book(book_id: Annotated[UUID, Path()], book_data: Annotated[UpdateBook, Query()]):
    book_id_str = str(book_id)
            
    result = await edit_book(book_id_str, book_data.model_dump(mode='json'))
    if result:
        return {'messages':f'Book with ID {book_id} update successfully'}
    raise HTTPException(404, 'Book not found')


# Delete Book
@app.delete('/delete-book/{book_id}/', tags=['Library'])
async def delete_book(book_id: Annotated[UUID, Path(title='The ID of book')]):
    book_id_str = str(book_id)
    
    list_books = read_book()
    book_exists = any(str(book.get('id')) == book_id_str for book in list_books)
    
    if not book_exists:
        raise HTTPException(status_code=404, detail=f'Book with ID {book_id} not found')
    
    deleted_id = await delete_book_new(book_id_str)
    
    if deleted_id:
        return {
            'message':f'Book with ID {book_id} deleted successfully'
        }
    else:
        raise HTTPException(status_code=500, detail='Failed to delete book')